...
muIsoExtractorCalo_(nullptr),muIsoExtractorTrack_(nullptr),muIsoExtractorJet_(nullptr)
{
  ...
  /*doOffline_(iConfig.getUntrackedParameter<bool>("doOffline"))
	{
   ...
   //edm::InputTag Tracks("generalTracks");  //inner tracks
   //tracksToken_ = consumes<std::vector<reco::Track>>(Tracks);


   //edm::InputTag Tracks("generalTracks");  //inner tracks

   //innerTrackCollectionToken_ = consumes<reco::TrackCollection>(Tracks);


   //edm::InputTag Links("links");
   //linksToken_ = consumes<reco::MuonTrackLinksCollection>(Links);
...
//int MuonIdProducer::sharedHits_frac_sharedHitsPixel(const reco::TrackExtra& track1, const reco::Track& track2) const {
//}
...
/*  edm::Handle<std::vector<reco::Track> > generalTracks;
  iEvent.getByToken(tracksToken_, generalTracks);

  edm::Handle<reco::MuonTrackLinksCollection > links;
  iEvent.getByToken(linksToken_, links);*/
   ...
   for ( unsigned int i = 0; i < inputCollectionLabels_.size(); ++i ) {
      ...
      if ( inputType == ICTypes::INNER_TRACKS ) {
       ...
       for(reco::TrackCollection::const_iterator itTracki = innerTrackCollectionHandle_->begin();
        ...
        //int sharedhits = sharedHits(*itTracki,*itTracki);  ////For Same Track
 ...
 /* for(std::vector<reco::Track>::const_iterator itTracki = generalTracks->begin();
      itTracki != generalTracks->end();
      ++itTracki) {


        //reco::TrackRef tki = itTracki->trackerTrack();


      for(std::vector<reco::Track>::const_iterator itTrackj = generalTracks->begin();
      itTrackj != generalTracks->end();
      ++itTrackj) {

        //reco::TrackRef tkj = itTrackj->trackerTrack();


//     int sharedhits = sharedHits(*tki,*tkj);
...
/*      for(unsigned int l1(0); l1 <links->size(); ++l1){
        const reco::MuonTrackLinks* link = &links->at(l1);
        reco::TrackRef tk1 = link->trackerTrack();

	for(unsigned int l2(0); l2 <links->size(); ++l2){
        const reco::MuonTrackLinks* link = &links->at(l2);
        reco::TrackRef tk2 = link->trackerTrack();

	int numHitsShared=sharedHits(*tk1, *tk2);

     std::cout<<"SharedHits:"<<numHitsShared<<std::endl;
}
}*/



/*      for(unsigned int l1(0); l1 <tracks->size(); ++l1){
       const reco::Track* track = &tracks->at(l1);
 //    const reco::trackerTracks* track = &tracks->at(l1);

        reco::TrackRef tk1 = track->trackerTrack();

        for(unsigned int l2(0); l2 <tracks->size(); ++l2){
        const reco::Track* track = &tracks->at(l2);
//      const reco::trackerTracks* track = &tracks->at(l2);

        reco::TrackRef tk2 = track->trackerTrack();
